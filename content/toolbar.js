(() => {
  ...
  const formatSaveBackendFeedback = (savedPrompt) => {
    ...
    if (backend === "anthropic")
      return "Prompt saved and optimized with Claude.";
    if (backend === "openrouter")
      return "Prompt saved and optimized with OpenRouter.";
  ...
  const refreshFabActionAvailability = async (root, platform) => {
    ...
    root.querySelectorAll(".pn-fab-action").forEach((button) => {
      ...
      if (action === "save-prompt" || action === "improve-prompt") {
        setFabActionUnsupported(
          button,
          hasInput ? "" : "No editable chat input found on this page.",
        );
        return;
      }
  ...
  const normalizeFabSettings = (value) => {
    ...
    const incomingButtons =
      source.fabButtons && typeof source.fabButtons === "object"
        ? source.fabButtons
    ...
    return {
      fabPosition:
        source.fabPosition === "bottom-left" || source.fabPosition === "left"
          ? "bottom-left"
          : "bottom-right",
      fabStyle: ["circle", "pill", "icon-only"].includes(
        String(source.fabStyle || ""),
      )
        ? source.fabStyle
        : "circle",
      fabButtons: {
  ...
  const applyFabSettings = (root, settings) => {
    ...
    root.classList.toggle(
      "pn-fab-left",
      normalized.fabPosition === "bottom-left",
    );
    root.classList.toggle(
      "pn-fab-right",
      normalized.fabPosition !== "bottom-left",
    ...
    root.classList.toggle(
      "pn-fab-style-circle",
      normalized.fabStyle === "circle",
    );
    root.classList.toggle("pn-fab-style-pill", normalized.fabStyle === "pill");
    root.classList.toggle(
      "pn-fab-style-icon-only",
      normalized.fabStyle === "icon-only",
    );
    ...
    const visibleActions = Array.from(
      root.querySelectorAll(".pn-fab-action"),
    ).filter((button) => {
    ...
    if (!saved) {
      ...
      if (window.Store?.isQuotaError?.(storageError)) {
        await showNotification(
          "Storage quota exceeded. Delete older prompts/history, then try again.",
        );
      } else {
      ...
      tagBadgeInput.addEventListener("keydown", (e) => {
        ...
        if (e.key === "Backspace" && !tagBadgeInput.value) {
          const badges = modal.querySelectorAll(
            "#pn-tag-badges-wrap .pn-tag-badge",
          );
  ...
  const toggleFabMenu = async (nextOpen = !isFabMenuOpen) => {
    ...
    const totalCloseMs =
      FAB_CLOSE_BASE_MS + actions.length * FAB_ACTION_STAGGER_MS;
    if (fabMenuCloseTimer) {
      clearTimeout(fabMenuCloseTimer);
    }
  ...
  const onExportClick = async (platform) => {
    try {
      ...
      if (
        typeof window.__PN?.SidePanelExport?.getSelectedMessages === "function"
      ) {
      ...
      if (messages.length === 0) {
        if (
          typeof window.__PN?.SidePanelExport?.activateSelectionModeAll ===
          "function"
        ) {
          const activated =
            await window.__PN.SidePanelExport.activateSelectionModeAll();
          if (activated) {
      ...
      const persisted = await chrome.runtime
      ...
      chrome.runtime.sendMessage({ action: "openExport" }, (response) => {
        if (chrome.runtime.lastError) {
          console.warn(
            "[Promptium] Could not open export panel:",
            chrome.runtime.lastError.message,
          );
          showNotification(
            "Could not open export panel. Open Promptium from the extension icon.",
          ).catch(console.error);
        }
      });
    } catch (error) {
  ...
  const onLibraryClick = () => {
    chrome.runtime.sendMessage({ action: "openSidePanel" }, () => {
      if (chrome.runtime.lastError) {
        showNotification("Open Promptium from the extension icon.").catch(
          console.error,
        );
      }
    });
  ...
  const onContinueChatClick = async () => {
    try {
      const response = await chrome.runtime
        .sendMessage({ action: "openContinuationPanel" })
        .catch(() => null);
      if (!response?.ok) {
        await showNotification(
          response?.error || "Could not open Continue Chat.",
        );
        return;
      }
    ...
    if (msg.action === "APPLY_IMPROVED_PROMPT" && msg.text) {
      void (async () => {
        ...
        const targetPlatform =
          String(msg.platform || runtimePlatform || "").trim() || null;
        const injected = await window.Injector.inject(
          String(msg.text),
          targetPlatform,
        );
        if (injected) {
    ...
    actions.forEach((actionButton) => {
      actionButton.addEventListener("click", (event) => {
        ...
        if (actionButton.classList.contains("pn-fab-action--unsupported")) {
          const reason = String(
            actionButton.dataset.unsupportedReason ||
              "Action unavailable on this page.",
          ).trim();
