...
const getEntryForMessage = (messageIndex, messageHash) => currentBookmarks.find((entry) => (
  Number(entry?.messageIndex) === Number(messageIndex) && String(entry?.messageHash || '') === String(messageHash || '')
));
...
const persistBookmarks = async () => {
  ...
  const snapshot = await chrome.storage.local.get([STORAGE_KEY]).catch(() => ({}));
  const all = snapshot?.[STORAGE_KEY] && typeof snapshot[STORAGE_KEY] === 'object' ? snapshot[STORAGE_KEY] : {};
...
const toggleBookmark = async (messageIndex, messageEl) => {
  ...
  const existingIndex = currentBookmarks.findIndex((entry) => (
    Number(entry?.messageIndex) === payload.messageIndex && String(entry?.messageHash || '') === payload.messageHash
  ));
  ...
  currentBookmarks = currentBookmarks.filter((entry) => Number(entry?.messageIndex) !== payload.messageIndex);
  ...
  messages.forEach(({ node: messageEl, conversationIndex }) => {
    ...
    if (!icon) {
      ...
      icon.addEventListener('click', (event) => {
        ...
        void toggleBookmark(Number.isFinite(messageIndex) ? messageIndex : conversationIndex, messageEl);
      });
...
const initShortcut = () => {
  document.addEventListener('keydown', (event) => {
    ...
    const isEditable = target instanceof HTMLElement && (
      ...
      (target instanceof HTMLInputElement && !['button', 'checkbox', 'radio', 'submit', 'range', 'file'].includes(String(target.type || '').toLowerCase()))
    );
