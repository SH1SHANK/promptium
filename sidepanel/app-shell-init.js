...
const registerEarlyListeners = () => {
  ...
  chrome.storage.onChanged.addListener((changes) => {
    ...
    if (!state.initialized) {
      state.pendingActions.push({ type: 'improvePayload', payload: improveChange.newValue });
      return;
    }
...
const init = async () => {
  window.PromptsUI.setCallbacks({
    onOpenImprove: (promptId, text, tags, options = {}) => window.ImproveUI.open(promptId, text, tags, options),
  ...
  window.SettingsAI.setCallbacks({
    onApplyExportDefaults: (settings) => window.ExportPayloadUI.applyDefaultsFromSettings(settings),
  ...
  if (state.exportSnapshotPayload?.messages?.length) {
    ...
    await window.ExportPayloadUI.setStatus('Select messages in chat, then click Export Selected.', false);
  }
