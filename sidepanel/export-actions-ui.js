...
const selectMessagesForExport = async () => {
  ...
  if (!context.tabId) {
    await window.ExportPayloadUI.setStatus('No active tab available for message selection.', true);
    return;
  }

  const response = await chrome.tabs.sendMessage(context.tabId, { action: 'openSidePanelAll' }).catch(() => null);
  if (!response?.ok) {
    await window.ExportPayloadUI.setStatus('Selection request failed on active tab.', true);
    return;
  }
...
const getCurrentBridgePlatform = async () => {
  const payloadPlatform = String(window.ExportPayloadUI.getActivePayload()?.platform || '').toLowerCase();
  if (payloadPlatform) return payloadPlatform;

  const context = await getActiveTabContext();
  if (!context.tabId) return '';
  const response = await chrome.tabs.sendMessage(context.tabId, { action: 'getPlatform' }).catch(() => null);
...
const getBridgeMessagesFromExport = async () => {
  ...
  const scraped = await chrome.tabs.sendMessage(context.tabId, { action: 'scrapeForBridge' }).catch(() => null);
