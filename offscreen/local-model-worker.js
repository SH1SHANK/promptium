...
const DEFAULT_LOCAL_MODEL_ID = "smollm2_1_7b";
const DEFAULT_EMBEDDING_MODEL_ID = "all-minilm-l6-v2";
...
const MODEL_CACHE_INDEX = {
  ...
  phi35_mini: {
    cacheNames: ["transformers-cache"],
    urlPatterns: ["microsoft/phi-3.5-mini-instruct", "phi-3.5-mini-instruct"],
  },
  qwen3_0_6b: {
    cacheNames: ["transformers-cache"],
    urlPatterns: ["qwen/qwen3-0.6b-instruct", "qwen3-0.6b-instruct"],
  },
};
...
const MODEL_STATE = Object.fromEntries(
...
const EMBEDDING_REGISTRY = Object.freeze(
  Object.fromEntries(
...
const EMBEDDING_CACHE_INDEX = Object.freeze(
  Object.fromEntries(
    EMBEDDING_MODELS.map((entry) => [
      entry.id,
      {
...
const EMBEDDING_STATE = Object.fromEntries(
  Object.keys(EMBEDDING_REGISTRY).map((key) => [
    key,
    {
      status: STATUS_TYPES.NOT_DOWNLOADED,
      progress: 0,
...
let activeEmbeddingModelId = String(
  getDefaultEmbeddingModel()?.id || "all-minilm-l6-v2",
);
let saveCacheIndexTimer = null;
let saveEmbeddingCacheIndexTimer = null;
let inMemoryCacheIndex = null;
let inMemoryEmbeddingCacheIndex = null;
let resolvedDevice = "wasm";
let deviceDetectionPromise = null;
...
const detectDevice = async () => {
  try {
    if (globalThis?.navigator?.gpu) {
      ...
      resolvedDevice = adapter ? "webgpu" : "wasm";
      return;
    }
  } catch (_error) {
...
const ensureDeviceDetection = async () => {
  if (hasDetectedDevice) {
    return resolvedDevice;
  }

  if (deviceDetectionPromise) {
    await deviceDetectionPromise;
    return resolvedDevice;
  }

  deviceDetectionPromise = (async () => {
    await detectDevice();
  })();
...
const resolveModelKey = (message, payload = {}) => {
  const explicit = String(payload?.modelId || message?.modelId || "")
    .trim()
    .toLowerCase();
  if (MODEL_REGISTRY[explicit]) return explicit;

  const fromSettings = String(
    message?.settings?.localModelId || DEFAULT_LOCAL_MODEL_ID,
  )
    .trim()
    .toLowerCase();
  return MODEL_REGISTRY[fromSettings] ? fromSettings : DEFAULT_LOCAL_MODEL_ID;
};

const resolveEmbeddingModelId = (message, payload = {}) => {
  ...
  const requested = getEmbeddingModelById(explicit);
  if (requested?.id) return requested.id;

  const fromSettings = String(
    message?.settings?.embeddingModelId ||
      activeEmbeddingModelId ||
      DEFAULT_EMBEDDING_MODEL_ID,
  ).trim();
  const resolved = getEmbeddingModelById(fromSettings);
  return String(
    resolved?.id ||
...
const ensureIndexShape = (raw) => {
  ...
  Object.keys(base).forEach((key) => {
    ...
    const cacheNames = Array.isArray(entry.cacheNames)
      ? entry.cacheNames
    ...
    base[key] = {
      ...
      urlPatterns: Array.from(
        new Set([...base[key].urlPatterns, ...urlPatterns]),
      ),
    };
  });
...
const readCacheIndex = async () => {
  if (!inMemoryCacheIndex) {
    inMemoryCacheIndex = ensureIndexShape(MODEL_CACHE_INDEX);
  }
  return ensureIndexShape(inMemoryCacheIndex);
};

const writeCacheIndex = async (index) => {
  inMemoryCacheIndex = ensureIndexShape(index);
};
...
const readEmbeddingCacheIndex = async () => {
  if (!inMemoryEmbeddingCacheIndex) {
    inMemoryEmbeddingCacheIndex = ensureEmbeddingIndexShape(
      EMBEDDING_CACHE_INDEX,
    );
  }
  return ensureEmbeddingIndexShape(inMemoryEmbeddingCacheIndex);
};

const writeEmbeddingCacheIndex = async (index) => {
  inMemoryEmbeddingCacheIndex = ensureEmbeddingIndexShape(index);
};
...
const withProgressCallback = (modelId, status) => (progressEvent) => {
  const state = MODEL_STATE[modelId];
  ...
  const dataStatus = String(progressEvent?.status || "").toLowerCase();
  ...
  if (dataStatus === "downloading") {
    ...
    progress = clamp(Math.round((loaded / total) * 100), 0, 100);
  } else {
    const reported = Number(progressEvent?.progress);
    if (!Number.isFinite(reported)) return;
    progress = clamp(Math.round(reported), 0, 100);
  }
  ...
  const fileHint = String(
    progressEvent?.file || progressEvent?.name || progressEvent?.url || "",
  ).trim();
  if (fileHint) {
...
const loadPipelineForModel = async (modelId, { downloadOnly = false } = {}) => {
  ...
  state.loadingPromise = (async () => {
    let loaded = null;
    try {
      await ensureDeviceDetection();
      const preferredDevice = resolvedDevice;
      try {
        state.backend = preferredDevice;
      ...
      state.error = String(
        error?.message || "Local model failed to initialize.",
      );
      state.progress = 0;
      await emitStatus(modelId);
      throw error;
...
const withEmbeddingProgressCallback = (modelId, status) => (progressEvent) => {
  const state = EMBEDDING_STATE[modelId];
  if (!state) return;
  if (state.cancelRequested) {
...
const loadPipelineForEmbeddingModel = async (
  modelId,
  { downloadOnly = false } = {},
) => {
  const state = EMBEDDING_STATE[modelId];
    ...
    try {
      ...
      try {
        ...
        loaded = await pipeline(config.task, config.modelId, {
          device: preferredDevice,
          dtype: config.dtype,
          progress_callback: withEmbeddingProgressCallback(
            modelId,
            state.status,
          ),
        });
      ...
      state.status = STATUS_TYPES.ERROR;
      state.error = String(
        error?.message || "Embedding model failed to initialize.",
      );
      state.progress = 0;
      await emitEmbeddingStatus(modelId);
      throw error;
...
const embedWithModel = async (modelId, text = "") => {
  ...
  if (!source) return [];
  const pipe = await loadPipelineForEmbeddingModel(modelId, {
    downloadOnly: false,
  });
...
const batchEmbedWithModel = async (
  modelId,
  texts = [],
  ratePerSecond = EMBEDDING_RATE_LIMIT_PER_SEC,
) => {
  ...
  const intervalMs = Math.max(
    1,
    Math.round(
      1000 / Math.max(1, Number(ratePerSecond) || EMBEDDING_RATE_LIMIT_PER_SEC),
    ),
  );
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    if (!text) {
      ...
      const vector = Array.from(output?.data || []).map(
...
const estimateFallbackClarity = (text) => {
  ...
  const hasConstraint =
    /(max|min|at least|no more than|format|tone|style|audience|length|steps|table|json|markdown)/i.test(
      source,
    );
...
const buildImprovePrompt = (text, tags = [], style = "general") => {
  ...
  const styleInstruction =
    styleMap[String(style || "general").toLowerCase()] || styleMap.general;
...
const buildContinuationPrompt = (
  ...
  const transcript = (Array.isArray(messages) ? messages : [])
    ...
    .map(
      (message) =>
        `${normalizeRole(message?.role)}: ${clampText(message?.text || "", 1800)}`,
    )
    .filter(Boolean)
    .join("\n\n");
...
const runTask = async (modelId, task, payload = {}) => {
  ...
  if (task === "paraphrase") {
    ...
    const rewritten = await runGeneration(modelId, prompt, {
      maxNewTokens: 320,
      temperature: 0.25,
      topP: 0.92,
    });
  ...
  if (task === "improve") {
    ...
    const improved = await runGeneration(modelId, prompt, {
      maxNewTokens: 520,
      temperature: 0.3,
      topP: 0.92,
    });
  ...
  if (task === "title") {
    ...
    const rawTitle = await runGeneration(modelId, prompt, {
      maxNewTokens: 40,
      temperature: 0.15,
      topP: 0.88,
    });
  ...
  if (task === "clarity") {
    ...
    const raw = await runGeneration(modelId, prompt, {
      maxNewTokens: 130,
      temperature: 0.1,
      topP: 0.85,
    });
    ...
    const explanation =
      ...
      estimateFallbackClarity(text).explanation;
    return { score, explanation };
  }
  ...
  if (task === "continue_summary") {
    ...
    const summarized = await runGeneration(modelId, prompt, {
      maxNewTokens: 520,
      temperature: 0.25,
      topP: 0.9,
    });
    return {
      text: limitWords(
        summarized ||
          "We were working on this conversation. Continue from here:",
  ...
  if (task === "tags") {
    ...
    const parsed =
      safeJsonParse(raw) ||
      safeJsonParse(String(raw || "").match(/\[[\s\S]*\]/)?.[0] || "");
...
const removeCachedModelData = async (modelId) => {
  ...
  const modelIdToken = String(
    MODEL_REGISTRY?.[modelId]?.modelId || "",
  ).toLowerCase();
  const looseTokens = [
    modelIdToken,
  ...
  for (const cacheName of cacheNames) {
    ...
    for (const request of requests) {
      ...
      const patternMatch = (entry.urlPatterns || []).some((pattern) =>
        url.includes(String(pattern || "").toLowerCase()),
      );
      const tokenMatch = looseTokens.some(
        (token) => token && url.includes(token),
      );
  ...
  return {
    ok: false,
    deletedRequests: 0,
    error: "No matching cached files found.",
  };
};
...
const removeCachedEmbeddingModelData = async (modelId) => {
  ...
  const entry = index?.[modelId] || { cacheNames: [], urlPatterns: [] };
  const modelIdToken = String(
    EMBEDDING_REGISTRY?.[modelId]?.modelId || "",
  ).toLowerCase();
  const looseTokens = [
    modelIdToken,
    modelIdToken.replace(/\//g, "%2f"),
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  void (async () => {
    ...
    const type =
      incomingType === "localModel:status"
    ...
    const payload =
      message?.payload && typeof message.payload === "object"
        ? message.payload
        : {};
    const modelId = resolveModelKey(message, payload);
    const embeddingModelId = resolveEmbeddingModelId(message, payload);
    ...
    try {
      ...
      if (type === "AI_EMBEDDING_DOWNLOAD") {
        await loadPipelineForEmbeddingModel(embeddingModelId, {
          downloadOnly: true,
        });
        sendResponse({
          ok: true,
      ...
      if (type === "AI_EMBEDDING_EMBED_TEXT") {
        const vector = await embedWithModel(
          embeddingModelId,
          String(payload.text || payload.input || ""),
        );
        sendResponse({
          ok: true,
          result: {
      ...
      if (type === "AI_LOCAL_MODEL_CANCEL_DOWNLOAD") {
        ...
        sendResponse({
          ok: true,
          result: { modelId, status: MODEL_STATE[modelId].status },
        });
        return;
      }
      ...
      sendResponse({
        ok: false,
        error: `Unsupported offscreen request type: ${type || "unknown"}`,
      });
    } catch (error) {
      sendResponse({
        ok: false,
