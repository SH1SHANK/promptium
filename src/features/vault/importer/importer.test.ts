...
async function runTests() {
  ...
  assert(
    detectImportSource('file.txt', '# CLAUDE.md') === 'claude',
    'Detects CLAUDE.md by content header.'
  );
  ...
  assert(
    detectImportSource('rules.json', '{"rules": []}') === 'cursor',
    'Detects cursor rules from JSON contents.'
  );
  assert(detectImportSource('test.md', '') === 'markdown', 'Detects generic markdown files.');

  // 2. Test Classifier & Confidence Levels
  const resultHigh = classifyContent(
    'Software Architect Profile',
    'You are an expert software architect...',
    'skills.sh'
  );
  assert(
    resultHigh.type === 'skill' && resultHigh.confidence >= 0.85,
    'High confidence for auto-detected skills.'
  );

  const resultMed = classifyContent(
    'TypeScript Coding Guide',
    'You should prefer using strict typescript type annotations.',
    'style.md'
  );
  assert(
    resultMed.type === 'instruction' && resultMed.confidence >= 0.6 && resultMed.confidence < 0.85,
    'Medium confidence for instructions.'
  );

  const resultLow = classifyContent(
    'Plain document',
    'This is some text about a topic with no instruction verbs or role descriptors.',
    'generic.txt'
  );
  assert(resultLow.confidence < 0.6, 'Low confidence for ambiguous plain text.');
...
- Avoid any type assignments.
  ...
  assert(
    preferredType === 'instruction',
    'Retrieved preference from local override learning cache.'
  );
