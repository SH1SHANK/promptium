(() => {
  ...
  const normalizeExportPrefs = (raw: any, fallback = state.exportPrefs || {}) => {
    ...
    return {
      ...
      exportStyle: ['standard', 'research', 'documentation'].includes(
        String(source.exportStyle || '').toLowerCase()
      )
        ? String(source.exportStyle).toLowerCase()
        : ['standard', 'research', 'documentation'].includes(
              String(base.exportStyle || '').toLowerCase()
            )
          ? String(base.exportStyle).toLowerCase()
          : 'standard',
      includeDate:
        source.includeDate !== undefined ? Boolean(source.includeDate) : Boolean(base.includeDate),
      includePlatform:
        source.includePlatform !== undefined
          ? Boolean(source.includePlatform)
          : Boolean(base.includePlatform),
      includeMessageNumbers:
        source.includeMessageNumbers !== undefined
          ? Boolean(source.includeMessageNumbers)
          : Boolean(base.includeMessageNumbers),
      includeThinking:
        source.includeThinking !== undefined
          ? Boolean(source.includeThinking)
          : Boolean(base.includeThinking),
      trimFollowUps:
        source.trimFollowUps !== undefined
          ? Boolean(source.trimFollowUps)
          : base.trimFollowUps !== undefined
            ? Boolean(base.trimFollowUps)
            : true,
      metadataPosition: ['header', 'footer', 'none'].includes(
        String(source.metadataPosition || '').toLowerCase()
      )
        ? String(source.metadataPosition).toLowerCase()
        : ['header', 'footer', 'none'].includes(String(base.metadataPosition || '').toLowerCase())
          ? String(base.metadataPosition).toLowerCase()
          : 'footer',
      contentMode:
        String(source.contentMode || base.contentMode || 'structured') === 'combined'
          ? 'combined'
          : 'structured',
      ...
      background: ['dark', 'light', 'sepia', 'custom'].includes(
  ...
  const hydrateExportPrefsFromStorage = async () => {
    ...
    const styleNode = byId('export-style');
    ...
    if (styleNode) styleNode.value = state.exportPrefs.exportStyle || 'standard';
    ...
    if (includePlatformNode)
      includePlatformNode.checked = Boolean(state.exportPrefs.includePlatform);
    ...
    if (includeThinkingNode)
      includeThinkingNode.checked = Boolean(state.exportPrefs.includeThinking);
    if (trimFollowUpsNode) trimFollowUpsNode.checked = Boolean(state.exportPrefs.trimFollowUps);
    if (metadataPositionNode)
      metadataPositionNode.value = String(state.exportPrefs.metadataPosition);
  ...
  const resolveExportThemeColors = () => {
    ...
    if (
      choice === 'custom' &&
      /^#([0-9a-f]{6}|[0-9a-f]{3})$/i.test(String(state.exportPrefs.customBackground || ''))
    ) {
      return {
        page: String(state.exportPrefs.customBackground),
        text: '#f5f5f5',
  ...
  const getExportFontClass = () => {
    ...
    if (selected.includes('georgia') || selected.includes('merriweather'))
      return 'pn-export-font--serif';
  ...
  const buildMarkdown = async () => {
    ...
    const exportStyle = String(state.exportPrefs.exportStyle || 'standard');
    if (exportStyle === 'research') {
      lines.push('Style: Research Notes');
    } else if (exportStyle === 'documentation') {
      lines.push('Style: Documentation');
    }
...
`;
  ...
  const buildExporterPrefs = () => {
    ...
    const exportStyle = String(state.exportPrefs.exportStyle || 'standard');
    const headerText =
      exportStyle === 'research'
        ? 'Research Notes'
        : exportStyle === 'documentation'
          ? 'Documentation'
          : '';
  ...
  const buildTextPreviewMarkup = async (format: string) => {
    ...
    if (
      !window.Exporter?.toTXT ||
      !window.Exporter?.toJSON ||
      !window.Exporter?.toNotion ||
      !window.Exporter?.toObsidian
    ) {
      return buildEmptyPreviewState({
  ...
  const syncPrefsFromControls = async () => {
    ...
    const exportStyle = byId('export-style');
    ...
    const sizeInput = Number(
      fontSize?.value || fontSizeNumber?.value || state.exportPrefs.fontSize || 14
    );
    ...
    state.exportPrefs = {
      ...
      exportStyle: ['standard', 'research', 'documentation'].includes(
        String(exportStyle?.value || '').toLowerCase()
      )
        ? String(exportStyle.value).toLowerCase()
        : 'standard',
      ...
      metadataPosition: String(
        metadataPosition?.value || state.exportPrefs.metadataPosition || 'footer'
      ),
      ...
      customBackground: String(
        customBackground?.value || state.exportPrefs.customBackground || '#18181c'
      ),
    };
  ...
  const renderMeta = async () => {
    ...
    if (selectionMeta) {
      ...
      if (!count) {
        ...
        selectionMeta.textContent = state.hasPendingExportUpdate
          ? `${base} • New selection available`
          : base;
      }
    }
    ...
    if (previewLabel) {
      const modeLabel =
        state.exportPrefs.contentMode === 'combined' ? 'combined text' : 'structured';
  ...
  const applyDefaultsFromSettings = (settings: any) => {
    ...
    if (styleNode) styleNode.value = state.exportPrefs.exportStyle;
    if (contentModeNode) contentModeNode.value = state.exportPrefs.contentMode;
