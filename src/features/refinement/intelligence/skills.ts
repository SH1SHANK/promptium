...
export const getSkillPack = (category: PromptCategory, promptText?: string): SkillPack => {
  ...
  const findSkill = (titleKeyword: string, fallbackId: string) => {
    ...
    const matched = vaultSkills.find(
      (s) => s.enabled && s.title.toLowerCase().includes(titleKeyword)
    );
  ...
  switch (category) {
    ...
    case 'business':
      if (
        ...
        normalizedText.includes('venture') ||
        normalizedText.includes('investor')
      ) {
