import { SUPPORTED_URLS } from './constants';
...
export const createEmptyState = (messageOrConfig: any, maybeOptions: any = {}) => {
  const isConfig =
...
export const createTagPill = (tag: any) => {
...
export const sortNodesByDomOrder = (nodes: any) =>
  Array.from(nodes || []).sort((a: any, b: any) =>
    a.compareDocumentPosition(b) & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : 1
  );

/** Returns true when URL is one of supported LLM hosts. */
export const isSupportedTabUrl = (url: any) => {
  const value = String(url || '').toLowerCase();
  return SUPPORTED_URLS.some((prefix: string) => value.startsWith(prefix));
};

/** Returns active tab metadata used for inject actions. */
export const getActiveTabContext = async () => {
...
export const sendToActiveTab = async (payload: any) => {
...
if (typeof window !== 'undefined') {
  ...
  (window as any).DomHelpers = DomHelpers;
}
