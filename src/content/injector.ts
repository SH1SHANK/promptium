(() => {
  ...
  const reactPlatforms = ['chatgpt', 'perplexity', 'claude'];
  ...
  const injectIntoReactTextarea = async (textarea, text) => {
    ...
    const proto =
      textarea instanceof HTMLTextAreaElement ? HTMLTextAreaElement.prototype : Element.prototype;
    const descriptor =
  ...
  const injectIntoEditable = async (editable, text) => {
    const isEditable =
      ...
      (editable.getAttribute('contenteditable') === 'true' ||
        editable.getAttribute('contenteditable') === 'plaintext-only');
    if (!isEditable) {
      return false;
    }
