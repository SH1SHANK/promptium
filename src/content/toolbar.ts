(() => {
    ...
    return {
      fabPosition:
        source.fabPosition === 'bottom-left' || source.fabPosition === 'left'
          ? 'bottom-left'
          : 'bottom-right',
      fabStyle: ['circle', 'pill', 'icon-only'].includes(String(source.fabStyle || ''))
        ? source.fabStyle
        : 'circle',
      fabButtons: {
  ...
  const setSaveModalDuplicateWarning = (scope = document, match = null) => {
    ...
    text.textContent = ' You can still save it, but this likely already exists in your library.';
  ...
  const renderSaveModalPreviewTokens = (container, text) => {
    ...
    while (match) {
      ...
      if (rawInner) {
        ...
        const varName = isOptional ? rawInner.slice(0, -1).trim() : rawInner;
        ...
        token.title = isOptional ? `Optional fill-in: ${varName}` : `Required fill-in: ${varName}`;
  ...
  const confirmSavePrompt = async () => {
    ...
    const duplicate = window.PromptDuplicate?.findDuplicate
      ...
      : null;
    setSaveModalDuplicateWarning(modal, duplicate?.duplicate ? duplicate.match : null);
  ...
  const bindSaveModalEvents = async () => {
    ...
    suggestTitleButton?.addEventListener('click', () => {
      void (async () => {
        ...
        try {
          const response = await window.AIBridge?.generatePromptTitle?.(pendingPromptText);
    ...
    try {
      ...
      if (messages.length === 0) {
        if (typeof window.__PN?.SidePanelExport?.activateSelectionModeAll === 'function') {
          const activated = await window.__PN.SidePanelExport.activateSelectionModeAll();
          if (activated) {
    ...
    actions.forEach((actionButton) => {
      actionButton.addEventListener('click', (event) => {
        ...
        if (actionButton.classList.contains('pn-fab-action--unsupported')) {
          const reason = String(
            actionButton.dataset.unsupportedReason || 'Action unavailable on this page.'
          ).trim();
